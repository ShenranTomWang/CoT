...
def load_model(model_name: str, device='cpu', dtype=torch.bfloat16) -> HookedTransformer:
    print(f"Loading model {model_name}...")
    weights_directory = "./models/" + config_model[model_name]['weights_directory']
    model = get_pretrained_model(weights_directory, dtype=dtype, device=device, verbose=True)
    return model
